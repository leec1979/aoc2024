...
fun main() {
    fun part1(input: List<String>): Int {
        val (firstList, secondList) = getSortedLists(input)

        // Diff values at each index - keep running sum
        var currSum = 0
        for (i in firstList.indices) {
            currSum += abs(firstList[i] - secondList[i])
        }
    ...
    fun part2(input: List<String>): Int {
        // Technically don't need the lists to be sorted
        val (firstList, secondList) = getSortedLists(input)
        val secondListCounts = mutableMapOf<Int, Int>()

        // Get the counts in the second list
        for (num in secondList) {
            secondListCounts[num] = secondListCounts.getOrDefault(num, 0) + 1
        }

        var similarityScore = 0
        for (num in firstList) {
            similarityScore += num * secondListCounts.getOrDefault(num, 0)
        }
...
fun getSortedLists(input: List<String>): Pair<List<Int>, List<Int>> {
    /**
     * Split the input into two lists. Insertion sort would be more efficient
     * than building the lists then sorting, but the lists aren't so big that matters
     * yet.
     */
    val firstList = mutableListOf<Int>()
    val secondList = mutableListOf<Int>()
    input.forEach { line ->
        val nums = line.split("\\s+".toRegex())
